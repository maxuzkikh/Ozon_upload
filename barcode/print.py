...
def print_pdfs_from_excel(excel_file_path, printer_name='Xprinter XP-365B', num_copies=1):
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            if isinstance(pdf_path, str) and os.path.exists(pdf_path) and pdf_path.lower().endswith('.pdf'):
                print_pdf_to_printer(pdf_path, acrobat_path, printer_name, num_copies)
            else:
                print(f"Error: Invalid or non-existent PDF path: {pdf_path}")
    except Exception as e:
        print(f"Error: Unable to process Excel file. {e}")

def print_pdf_to_printer(pdf_path, acrobat_path, printer_name='Xprinter XP-365B', num_copies=1):
    try:
        # Set width and height according to Xprinter property
        command = [acrobat_path, "/t", pdf_path, printer_name, f"/n {num_copies}", '/h', '40', '/w', '58', '/o', 'album']
...
# Optional: Specify the number of copies
num_copies = 2
...
print_pdfs_from_excel(excel_file_path, printer_name, num_copies)
