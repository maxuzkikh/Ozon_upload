...
price_before_col, _ = find_column(data_path, 'Лист1', 'Цена до скидки, руб.')
vat_col, _ = find_column(data_path, 'Лист1', 'НДС, %*')
promote_col, _ = find_column(data_path, 'Лист1', 'Включить продвижение')
ozon_ID_col, _ = find_column(data_path, 'Лист1', 'Ozon ID')
barcode_col, _ = find_column(data_path, 'Лист1', 'Штрихкод (Серийный номер / EAN)')
...
color2_name_col, _ = find_column(data_path, 'Лист1', 'Название цвета')
...
for i, row in enumerate(df.itertuples(), start=4):
    ...
    ws_output.cell(row=i, column=find_column(output_file_path, 'Шаблон', 'Цена до скидки, руб.')[0], value=row[price_before_col])
    ...
    ws_output.cell(row=i, column=find_column(output_file_path, 'Шаблон', 'Ozon ID')[0], value=row[ozon_ID_col])
    ws_output.cell(row=i, column=find_column(output_file_path, 'Шаблон', 'Штрихкод (Серийный номер / EAN)')[0], value=row[barcode_col])
    ...
    ws_output.cell(row=i, column=find_column(output_file_path, 'Шаблон', 'Цвет товара')[0], value=row[color_name_col])
    ws_output.cell(row=i, column=find_column(output_file_path, 'Шаблон', 'Название цвета')[0], value=row[color2_name_col])
